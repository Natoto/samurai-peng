...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.leftItem.center = CGPointMake( ITEM_HEIGHT/2 + 10,STATUS_HEIGHT + BAR_HEIGHT/2);
        self.rightItem.center = CGPointMake( [UIScreen mainScreen].bounds.size.width - ITEM_HEIGHT/2 - 10 , BAR_HEIGHT/2 + STATUS_HEIGHT);
...
-(void)drawbottomlinelayer
{
    CALayer *imageLayer = bottomLayer;
    if (!imageLayer) {
        CALayer *imageLayer =  [self createLayer:CGRectMake(0, self.frame.size.height - 0.5, UISCREEN_WIDTH, 0.5) color:KT_UIColorWithRGB(219, 219, 219)];
